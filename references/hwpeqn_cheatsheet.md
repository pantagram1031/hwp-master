...
항상 위·아래첨자 범위를 `{}`로 명시한다. HwpEqn은 중괄호 없는 `^`/`_` 뒤에서
다음 공백까지를 첨자로 잡을 수 있어 `x^2)=...`처럼 구두점과 다음 항까지 함께
올라가는 경우가 있다. 변환기는 단일 원자를 자동으로 감싸지만, 여러 글자 첨자는
작성 단계에서 `D_{pq'-qp'}`처럼 직접 묶는다. 새 인라인 수식은 페이지 썸네일만
보지 말고 고해상도로 첨자 범위를 확인한다.
